...
// Check if Tauri stream_fetch command is available
function hasTauriStreamFetch(): boolean {
  try {
    // For now, we'll use browser fetch since stream_fetch is not implemented in the backend
    // The Rust backend only has screenshot commands, not stream_fetch
    return false;
  } catch {
    return false;
  }
...
export function fetch(url: string, options?: RequestInit): Promise<Response> {
  if (window.__TAURI__ && hasTauriStreamFetch()) {
    const {
      signal,
      method = "GET",
      headers: _headers = {},
