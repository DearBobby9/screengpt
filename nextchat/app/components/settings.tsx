...
export function Settings() {
  ...
  const openAICompatibleConfigComponent = accessStore.provider ===
    ServiceProvider.OpenAICompatible && (
    <>
      <ListItem
        title="API Endpoint"
        subTitle="Enter your OpenAI compatible API endpoint (e.g., http://localhost:8080)"
      >
        <input
          aria-label="API Endpoint"
          type="text"
          value={accessStore.openaiCompatibleUrl}
          placeholder="https://api.example.com"
          onChange={(e) =>
            accessStore.update(
              (access) => (access.openaiCompatibleUrl = e.currentTarget.value),
            )
          }
        ></input>
      </ListItem>
      <ListItem
        title="API Key"
        subTitle="Enter your API key for the custom endpoint"
      >
        <PasswordInput
          aria-label="API Key"
          value={accessStore.openaiCompatibleApiKey}
          type="text"
          placeholder="sk-xxx"
          onChange={(e) => {
            accessStore.update(
              (access) => (access.openaiCompatibleApiKey = e.currentTarget.value),
            );
          }}
        />
      </ListItem>
      <ListItem
        title="Provider Name"
        subTitle="Display name for this provider in model selector"
      >
        <input
          aria-label="Provider Name"
          type="text"
          value={accessStore.openaiCompatibleProviderName}
          placeholder="Custom"
          onChange={(e) =>
            accessStore.update(
              (access) => (access.openaiCompatibleProviderName = e.currentTarget.value),
            )
          }
        ></input>
      </ListItem>
      <ListItem
        title="Fetch Models"
        subTitle="Fetch available models from your API endpoint"
      >
        <IconButton
          text="Fetch Models"
          onClick={async () => {
            if (!accessStore.openaiCompatibleUrl || !accessStore.openaiCompatibleApiKey) {
              showToast("Please enter API URL and API Key first");
              return;
            }
            try {
              showToast("Fetching models...");
              const api = new ClientApi(ModelProvider.OpenAICompatible);
              const models = await api.llm.models();
              if (models.length > 0) {
                accessStore.update((access) => {
                  access.openaiCompatibleFetchedModels = models.map((m) => m.name);
                });
                config.mergeModels(models);
                showToast(`Fetched ${models.length} models and added to model list`);
              } else {
                showToast("No models found");
              }
            } catch (e) {
              showToast("Failed to fetch models: " + (e as Error).message);
            }
          }}
        />
      </ListItem>
            ...
            <>
              ...
              {accessStore.useCustomConfig && (
                <>
                  ...
                  {openAICompatibleConfigComponent}
                </>
              )}
            </>
          )}
